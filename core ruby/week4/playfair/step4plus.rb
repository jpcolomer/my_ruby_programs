class Ciphersquare
  ...
  def initialize(key_phrase)
    ...
    @hash_table = Hash.new
    @chars = [*('a'..'z')].join('').upcase.gsub(/j/i,'I').split('').uniq
    @chars.each do |char1|
      @chars.each do |char2|
        @hash_table[char1+char2] = encrypt_pair(char1+char2)
      end
    end
  ...
  def encrypt_row (coord)
    @row, @col = coord
    @table[@row][move_index(@col, 1)]
  end

  def encrypt_col (coord)
    @row, @col = coord
    @table[move_index(@row, 1)][@col]
  end

  def decrypt_row (coord)
    @row, @col = coord
    @table[@row][move_index(@col, -1)]
  end

  def decrypt_col (coord)
    @row, @col = coord
    @table[move_index(@row, -1)][@col]
  end
...
playfair = Playfaire.new('We love ruby')
puts playfair.cipher_square.to_s
puts playfair.cipher_square.hash_table
=begin
P L A Y F
I R E X M
B C D G H
...
puts playfair.formed_message(input_message) #=> HIDETHEGOLDINTHETREXESTUMP
encrypted_message = playfair.encrypt_message(input_message)
puts encrypted_message #=> BMODZBXDNABEKUDMUIXMMOUVIF
puts playfair.decrypt_message(encrypted_message) #=> HIDETHEGOLDINTHETREXESTUMP
